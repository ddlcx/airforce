...
```mermaid
flowchart TD
    A["YOLO 检测结果"] --> B["提取 kp22(左顶)<br/>kp24(右顶)"]
    B --> C{"kp22/kp24<br/>置信度 ≥ 阈值?"}
    C -->|No| D["返回 None"]
    C -->|Yes| E["提取底部关键点<br/>kp23, kp25"]
    E --> F["输出 NetKeypointResult"]
...
从 YOLO 球网模型提取球网 4 个关键点：kp22（网左端顶部）、kp24（网右端顶部）、kp23（网左端底部）、kp25（网右端底部）。其中 kp22 和 kp24 为必需。

YOLO 球网模型本身精度很高，直接使用模型原始值。`extract_net_keypoints` 不依赖 Homography 矩阵，降低模块间耦合。
...
```
YOLO检测 → Homography → 球网关键点提取 → PnP
  Σ_kpt       J_H           J_net          J_pnp
```

每级的 Jacobian 放大因子决定了误差如何传播。关键发现：

- **H 对关键点的 Jacobian**：当点均匀分布在球场时，H 的误差与关键点误差同量级。当点聚集在一小片区域时，远离该区域的投影误差会放大。
- **球网关键点独立于 H**：球网关键点直接使用 YOLO 原始值，不经过 H 变换，避免了 H 噪声向球网关键点传播。
