...
为支持 YOLO 双路检测球网顶部线，额外定义4个球网关键点：

```
编号   名称              坐标 (X, Y, Z) 米
────   ────              ────────────────
22     网左端顶部        (-3.05, 0.00, 1.55)
23     网右端顶部        (+3.05, 0.00, 1.55)
24     网中心顶部        ( 0.00, 0.00, 1.524)
25     网中心底部(绳)    ( 0.00, 0.00, 0.00)
```

> 关键点24提供球网顶线的中点约束（球网中心比两端低26mm），25标记球网绳在地面的位置，增强 YOLO 对球网区域的感知能力。

## 5. 水平翻转对应 (flip_idx)

用于 YOLO 训练时的水平翻转数据增强。左右对称的关键点互换索引：

```
flip_idx = [1,0, 3,2, 5,4, 7,6, 9,8, 11,10, 13,12, 15,14, 17,16, 18,19, 20,21, 23,22, 24,25]
```

18、19（中线/前发球线交点）、20、21（底线中点）和24、25（网中心点）在翻转后保持不变。
